...
public class WorstFit extends Politicas {
    @Override
    public Resultado ejecutar(List<Trabajo> listaTrabajos, int tamanioMp, int tiempoCargaMp, int tiempoSelec, int tiempoLiberacion) throws InterruptedException {
        System.out.println("Opción Worst Fit seleccionada");
        ...
        while (!listaTrabajos.isEmpty()) {
                ...
                // Filtro y ordenar particiones las particiones libres de mayor a menor tamaño
                List<Particion> particionesLibres = listaParticiones.stream().filter(Particion::getEstado).sorted(Comparator.comparing(Particion::getTamanioParticion).reversed()).toList();

                for (Particion p : particionesLibres) {
                System.out.println("Particiones libres ordenadas de mayor a menor: [" + p + "]");
                }

                int i = 0;
                boolean buscoLugar = true;

                while (buscoLugar && i < particionesLibres.size()) {
                    Particion particionActual = particionesLibres.get(i);

                    if (particionActual.getTamanioParticion() >= trabajoActual.getTamanio()) {
                        // Se encontró el lugar entonces pasa a ser falso porque deja de buscar
                        buscoLugar = false;
                        
                        Particion particionOriginal = listaParticiones.stream().filter(p -> p.getId() == particionActual.getId()).findFirst().orElse(null);
                            ...
                            if (particionOriginal.getTamanioParticion() == trabajoActual.getTamanio()) {// si la partición es exacta creo una nueva partcion ocupada y reemplaza la original
                                ...
                                int indiceOriginal = listaParticiones.indexOf(particionOriginal);
        
                                //elimino la particion original
                                listaParticiones.remove(particionOriginal);
        
                                //inserto la nueva particicion en el mismo indice
                                listaParticiones.add(indiceOriginal, particionNueva);
                            }
                            else  if (particionOriginal.getTamanioParticion() > trabajoActual.getTamanio()){//si particion mas grande que la del trabajo la voy a dividir en 2 la 1ra asigno al trabajo                       
                                ...
                                Particion particionSobrante = new Particion(true,-1,-1,particionOriginal.getTamanioParticion() - trabajoActual.getTamanio(),-1);
                ...
                Particion particionConMayorTiempo = listaParticiones.stream().max((p1, p2) -> Long.compare(p1.getTiempoFinalizacion(), p2.getTiempoFinalizacion())).orElse(null);
